...
JOB_ID="$(curl -X POST -H 'Content-Type: application/json' -H "X-API-KEY: $PACKAGING_API_KEY" -d "{ \"owner\": \"$GITLAB_USER_EMAIL\", \"release_pkg\": 1, \"pkg_scm_url\": \"git@gitlab.booking.com:$CI_PROJECT_PATH.git\", \"pkg_scm_rev\":\"tag::$CI_COMMIT_TAG\", \"spec_scm_dir\": \"packages.git::$CI_PROJECT_NAME\", \"centos\": \"7,8\" }" http://yum-master.prod.booking.com/api/builds/new?src_from_scm=1 | tee /dev/fd/2 | jq .job_id)"
...
while true; do
    for centos_version in 7 8; do
        if [[ ${jobdone[$centos_version]} -eq 1 ]]; then
            continue
        fi
        ...
        [[ $(jq .build_state <<<$response) =~ done|failed ]] && echo ===centos${centos_version}=== finished && jobdone[$centos_version]=1
    done
    [[ ${jobdone[7]} -eq 1 && ${jobdone[8]} -eq 1 ]] && break
    sleep 15
done
